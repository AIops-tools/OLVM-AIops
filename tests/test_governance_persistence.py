...
throwaway home (``OLVM_AIOPS_HOME``) and assert that the rows compliance
...
import olvm_aiops.governance.policy as policy_mod
import olvm_aiops.governance.undo as undo_mod
from olvm_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OLVM_AUDIT_APPROVED_BY", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="engine1")
...
def test_high_risk_runs_without_an_approver_and_is_audited(gov_home):
    ...
    result = _drop_widget(name="w2", target="engine1")
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("OLVM_AUDIT_APPROVED_BY", "virt-alice")
    result = _drop_widget(name="w3", target="engine1")
    assert result["status"] == "dropped"
...
# The end-to-end "real product write tool persists priorState + undo" test is
# rebuilt on OLVM's first governed write tool, once its engine response shape
# has been captured from the live lab (the XCP-ng snapshot version was removed).
