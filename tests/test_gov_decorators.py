...
``olvm_aiops.governance.*`` and binds state to an isolated home via
``OLVM_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import olvm_aiops.governance.decorators as dec_mod
...
from olvm_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OLVM_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("OLVM_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("OLVM_MAX_TOOL_CALLS", "0")
