...
nothing touches the real ``~/.olvm-aiops``.
"""
...
def test_roundtrip_set_get(store_dir):
    ...
    store = store.set("engine1", "p@ssw0rd-123")
    reopened = ss.SecretStore.unlock("master-pw")
    assert reopened.get("engine1") == "p@ssw0rd-123"


def test_value_not_plaintext_on_disk(store_dir):
    ss.SecretStore.unlock("master-pw").set("engine1", "super-secret-value")
...
def test_migrate_legacy_env(store_dir):
    (store_dir / ".env").write_text(
        "OLVM_ENGINE1_PASSWORD=legacy-pass\n# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("OLVM_", "_PASSWORD", "pw")
    assert imported == ["engine1"]
    assert ss.SecretStore.unlock("pw").get("engine1") == "legacy-pass"
