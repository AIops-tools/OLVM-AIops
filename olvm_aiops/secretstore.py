"""Encrypted secret storage for olvm-aiops.

Secrets (OLVM engine account passwords) are NEVER written to disk in plaintext. They
live in ``~/.olvm-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``OLVM_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "olvm-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "OLVM_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``OLVM_<TARGET>_PASSWORD``; ``prefix='OLVM_'`` and
