...
Responsibilities:
  ...
  3. Post-log: write audit record to ``~/.olvm-aiops/audit.db``
...
from olvm_aiops.governance.audit import detect_agent, get_engine
from olvm_aiops.governance.budget import BudgetExceeded, get_budget
from olvm_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from olvm_aiops.governance.patterns import PatternMatch, get_pattern_engine
from olvm_aiops.governance.policy import get_policy_engine
from olvm_aiops.governance.sanitize import sanitize

_log = logging.getLogger("olvm-aiops.decorators")
    ...
    *,
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.olvm-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("OLVM_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("OLVM_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`olvm_aiops.governance.outcome`). The change may be live,
    ...
    if not descriptor:
        return
    try:
        from olvm_aiops.governance.undo import get_undo_store
...
def _infer_skill(func: Any) -> str:
    ...
    ``olvm_aiops.ops.jobs`` → ``olvm-aiops``
    ``mcp_server.server`` → ``olvm-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("olvm_aiops") or module.startswith("mcp_server"):
        return "olvm-aiops"
    return "unknown"
