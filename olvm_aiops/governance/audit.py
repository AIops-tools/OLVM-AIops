"""Unified audit logging engine — all olvm skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.olvm-aiops/audit.db``.
...
from olvm_aiops.governance.paths import ops_path

_log = logging.getLogger("olvm-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring OLVM_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the OLVM_AIOPS_HOME refactor
