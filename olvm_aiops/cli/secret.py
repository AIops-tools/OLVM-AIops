"""``olvm-aiops secret`` — manage the encrypted credential store.

Secrets (OLVM engine account passwords) are stored in ``~/.olvm-aiops/secrets.enc``
...
from olvm_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_set(
    ...
    value: Annotated[
        str | None,
        typer.Option("--value", help="Password (omit to be prompted, hidden)"),
    ] = None,
) -> None:
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: olvm-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update OLVM_AIOPS_MASTER_PASSWORD.[/]")
